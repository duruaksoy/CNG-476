...
void PacketForwarder::sendNextPacketToServer() {
    if (!pendingLoRaPackets.empty()) {
        waitingForServerResponse = true;
        Packet *pk = pendingLoRaPackets.front();
        pendingLoRaPackets.pop();

        // Prepare and send the packet (same as before)
        auto frame = pk->peekAtFront<LoRaMacFrame>();
...
//    socket.sendTo(pk, destAddr, destPort);
    pendingLoRaPackets.push(pk);
    totalPackets++;
    EV << "Pushed packet to queue" << endl;
    EV << "Queue size for MS: " << pendingLoRaPackets.size() << endl;
    // If not waiting for a response, send the next packet
    if (!waitingForServerResponse) {
        EV << "Sent packet to server" << endl;
        sendNextPacketToServer();}
}
...
void PacketForwarder::finish()
{
    ...
    recordScalar("total_packets_delivered", totalPackets);
}

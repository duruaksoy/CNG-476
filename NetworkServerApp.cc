...
void NetworkServerApp::handleMessage(cMessage *msg)
{
    if (msg->arrivedOn("socketIn")) {
        ...
        // Extract the LoRaAppPacket (if it exists)
        pkt->trimFront();
        auto loraMacFrame = pkt->removeAtFront<LoRaMacFrame>();
        const auto &rcvAppPacket = pkt->peekAtFront<LoRaAppPacket>();

        // Get the gateway address (where the packet came from)
        const auto &networkHeader = getNetworkProtocolHeader(pkt);
        const L3Address &gwAddress = networkHeader->getSourceAddress();

        // Create an acknowledgment packet (ACK)
        auto ackAppPacket = makeShared<LoRaAppPacket>();
        ackAppPacket->setMsgType(TXCONFIG);  // Or define a custom ACK type

        // Encapsulate in a MAC frame
        auto ackMacFrame = makeShared<LoRaMacFrame>();
        ackMacFrame->setChunkLength(B(par("headerLength").intValue()));
        ackMacFrame->setReceiverAddress(loraMacFrame->getTransmitterAddress());
        ackMacFrame->setLoRaTP(math::dBmW2mW(14));  // Default power
        ackMacFrame->setLoRaCF(loraMacFrame->getLoRaCF());
        ackMacFrame->setLoRaSF(loraMacFrame->getLoRaSF());
        ackMacFrame->setLoRaBW(loraMacFrame->getLoRaBW());

        // Build and send the ACK packet
        auto ackPacket = new Packet("ACK");
        ackAppPacket->setChunkLength(B(par("headerLength").intValue()));
        ackPacket->insertAtFront(ackAppPacket);
        ackPacket->insertAtFront(ackMacFrame);


//        socket.sendTo(ackPacket, gwAddress, destPort);

        // Create a timer message to delay the ACK
        auto *timerMsg = new AckTimerMessage("AckTimer", ackPacket);
        timerMsg->setGatewayAddress(gwAddress);
        timerMsg->setNodeAddress(loraMacFrame->getTransmitterAddress());

        // Generate random delay (adjust parameters as needed)
        delayCount++;
        simtime_t delay = uniform(0.1, 1.0);
        totalDelays += delay;
        scheduleAt(simTime() + delay, timerMsg);

        EV << "Scheduled ACK to node " << loraMacFrame->getTransmitterAddress() << " with delay " << delay << "s" << endl;

        // Delete the original packet (optional, if not needed anymore)
        delete pkt;
    }
    else if (msg->isSelfMessage()) {
//        processScheduledPacket(msg);
        // This is our delayed self-message, time to send the ACK
        auto *timerMsg = check_and_cast<AckTimerMessage*>(msg);
        Packet *ackPacket = timerMsg->getAckPacket();  // This transfers ownership
        if (ackPacket) {
            socket.sendTo(ackPacket, timerMsg->getGatewayAddress(), destPort);
            EV << "Sent delayed ACK to node " << timerMsg->getNodeAddress() << endl;
        }
        delete msg;  // Delete the timer message
    }
}
...
void NetworkServerApp::finish()
{
    ...
    recordScalar("total_delays", totalDelays.dbl());
    recordScalar("delay_count", delayCount);
